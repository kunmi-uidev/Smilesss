...
app.post("/api/gemini/generate-presentation", async (req, res) => {
  try {
    ...
    const prompt = `
      ...
      1. Keep the presentation super-fast and highly focused: generate exactly 5 to 7 slides maximum.
      ...
      3. For bullets, rewrite copy into 2 to 3 concise, professional, scannable statements (under 8 words per line). Never output wordy paragraphs.
      4. For each slide, write a single very brief, 1-sentence strategic focus 'note' (under 12 words) to keep generation latency ultra-low.
      5. Provide an relevant imageSearchQuery for each slide (e.g. "modern office", "abstract waves", "growth charts").
    ...
    const response = await retryWithBackoff(() =>
      ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: "You are an elite high-speed presentation designer. Consolidate and rewrite content to be extremely punchy, short, and clean to ensure ultra-low latency generation. Ensure response complies exactly with the requested JSON schema. Do not use markdown backticks inside JSON strings.",
          ...
          responseSchema: {
            ...
            properties: {
              ...
              slides: {
                type: Type.ARRAY,
                description: "List of carefully structured presentation slides (5-7 slides max for maximum performance).",
                items: {
                  type: Type.OBJECT,
                  properties: {
                    title: {
                    ...
                    content: {
                      ...
                      description: "2-3 short, punchy scannable bullet items under 8 words each.",
                    },
                    layout: {
                      type: Type.STRING,
